...
//初始化区域图的对象
	...
	if(property.haveTool){
     ...
     +"<span/>"
     +"<p style = 'text-align: center;font-size:20px'>输入</p>"
     );
		if(property.toolBtns&&property.toolBtns.length>0){
			...
			for(var i=0;i<property.toolBtns.length;++i){
				...
				if(this.$id+"_btn_"+property.toolBtns[i].split(" ")[0] == "demo_btn_http"){
					tmp+="<span/>";
					tmp+="<p style = 'text-align: center;font-size:20px'>输出</p>";
				}
				if(this.$id+"_btn_"+property.toolBtns[i].split(" ")[0] == "demo_btn_res"){
					tmp+="<span/>";
					tmp+="<p style = 'text-align: center;font-size:20px'>功能</p>";
				}
			}
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='width:"+(width1)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	this.$workArea=$("<div class='GooFlow_work_inner' style='width:"+width1+"px;height:"+height+"px'></div>")
